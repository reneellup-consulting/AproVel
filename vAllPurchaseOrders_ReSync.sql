...
/****** Object:  View [dbo].[vAllPurchaseOrders_ReSync]    Script Date: 02/26/2026 11:50:01 am ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER   VIEW [dbo].[vAllPurchaseOrders_ReSync] AS
...
SELECT 
	...
	g.IsSynced,
    g.ReSynced, -- Added ReSynced column
...
WHERE g.EntryDate >= '2025-01-01'
  -- Filter strictly for records that are synced AND flagged for re-sync
  AND g.IsSynced = 1 
  AND g.ReSynced = 1
...
SELECT 
    ...
    CASE 
        WHEN f.TruckOrGenset = 2 THEN 'None'               
        WHEN t.UnitNo IS NOT NULL THEN t.UnitNo            
    ...
    (
        SELECT STUFF((
            ...
            FROM [dbo].[vFuelTaggedTrips] trip  
            WHERE trip.HeaderID = f.OID        
            FOR XML PATH(''), TYPE
...
LEFT JOIN vFleetVehicles t ON t.Oid = 
    CASE 
        WHEN f.TruckOrGenset = 0 THEN f.TruckNo     
        WHEN f.TruckOrGenset = 1 THEN f.GensetNo    
        WHEN f.TruckOrGenset = 3 THEN f.OtherNo     
        ELSE NULL                                   
    END
